...
def main():
    ...
    model = Qwen3VLForConditionalGeneration.from_pretrained(
        cfg["model"]["name"], device_map="cuda", torch_dtype=getattr(torch, cfg["model"]["dtype"]), trust_remote_code=True,
    )
    ...
    total_params = sum(p.numel() for layer in model.model.language_model.layers for p in layer.parameters())
