...
from swiftsvd.modeling.modeling_qwen3_vl_svd import Qwen3VLForConditionalGenerationSVD
...
def main():
    ...
    new_model = Qwen3VLForConditionalGenerationSVD.from_pretrained(
    ...
    for name in module_names:
        for i in range(len(model.model.language_model.layers)):
            ...
            with torch.no_grad():
                ...
                if "mlp" in name:
                    getattr(model.model.language_model.layers[i].mlp, model_weight_name).weight.copy_(w_recon.T)
                    getattr(new_model.model.language_model.layers[i].mlp, model_weight_name + "_u").weight.copy_(u.T)
                    getattr(new_model.model.language_model.layers[i].mlp, model_weight_name + "_v").weight.copy_(v.T)
                else:
                    getattr(model.model.language_model.layers[i].self_attn, model_weight_name).weight.copy_(w_recon.T)
                    getattr(new_model.model.language_model.layers[i].self_attn, model_weight_name + "_u").weight.copy_(u.T)
                    getattr(new_model.model.language_model.layers[i].self_attn, model_weight_name + "_v").weight.copy_(v.T)
            del u, v
            gc.collect()
