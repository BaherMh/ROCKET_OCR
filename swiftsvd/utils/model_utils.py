...
def get_weight_transposed(model, name, layer_idx):
    if "mlp" in name:
        proj = name.split('.')[-1]
        return getattr(model.model.language_model.layers[layer_idx].mlp, proj).weight.T
    else:
        proj = name.split('.')[-1]
        return getattr(model.model.language_model.layers[layer_idx].self_attn, proj).weight.T
